...
export default async function runJsxToTsxConversion(inputFile) {
  try {
    const jsxCode = fs.readFileSync(inputFile, "utf-8");
    ...
    const output = generate(tsxAst);
    const outputFile = inputFile.replace(/\.jsx$/, '.tsx');

    fs.writeFileSync(outputFile, output.code, 'utf-8');
    console.log(`✅ Converted ${inputFile} to ${outputFile}`);
  } catch (err) {
    console.error('❌ Error during JSX-to-TSX conversion:', err);
    process.exit(1);
  }
}
...
// const [, , inputFile = './sample.jsx'] = process.argv;
// const jsxCode = fs.readFileSync(inputFile, "utf-8");
